...
let%expect_test "parse simple measure to the 3 power" =
  ...
  [%expect {| (Measure_pow ((Measure_ident "kg"), (Int_exp_pos 3))) |}]
;;

let%expect_test "parse simple measure to the negative 3 power" =
  pp pp_measure parse_measure {|kg^-3|};
  [%expect {| (Measure_pow ((Measure_ident "kg"), (Int_exp_neg 3))) |}]
;;
...
let%expect_test "parse measure without power as measure to the first power" =
  ...
  [%expect {| (Measure_pow ((Measure_ident "kg"), (Int_exp_pos 1))) |}]
;;
...
let%expect_test "parse two measure powers in sequence as product" =
  ...
  [%expect
    {|
    (Measure_prod ((Measure_pow ((Measure_ident "kg"), (Int_exp_pos 3))),
       (Measure_pow ((Measure_ident "cm"), (Int_exp_pos 3))))) |}]
;;
...
let%expect_test "parse three measure powers in sequence as left-associated product" =
  ...
  [%expect
    ...
    (Measure_prod (
       (Measure_prod ((Measure_pow ((Measure_ident "kg"), (Int_exp_pos 3))),
          (Measure_pow ((Measure_ident "cm"), (Int_exp_pos 3))))),
       (Measure_pow ((Measure_ident "lb"), (Int_exp_pos 1))))) |}]
;;
  ...
  [%expect
    ...
    (Measure_prod (
       (Measure_prod (
          (Measure_prod (
             (Measure_prod (
                (Measure_pow ((Measure_ident "kg"), (Int_exp_pos 1))),
                (Measure_pow ((Measure_ident "cm"), (Int_exp_pos 3))))),
             (Measure_pow ((Measure_ident "N"), (Int_exp_neg 3))))),
          (Measure_pow ((Measure_ident "v"), (Int_exp_pos 1))))),
       (Measure_pow ((Measure_ident "v"), (Int_exp_pos 4))))) |}]
;;
