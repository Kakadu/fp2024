...
let%expect_test "parse inverse as measure" =
  ...
  [%expect {|
    (Measure_div (Measure_dimless, (Measure_ident "m"))) |}]
;;
...
let%expect_test "parse something strange" =
  pp pp_measure pm {| (kg * cm^3 / (s^-1 / y^2))|};
  [%expect
    {|
    (Measure_div (
       (Measure_prod ((Measure_ident "kg"),
...
let%expect_test "parse uom 1" =
  pp pp_unit_of_measure puom {|421088.397651<(((bWY ^ 2) * 1) * (1 ^ 4))>|};
  [%expect
    {|
    (Unit_of_measure ((Mnum_float 421088.397651),
       (Measure_prod (
          (Measure_prod ((Measure_pow ((Measure_ident "bWY"), 2)),
             Measure_dimless)),
          (Measure_pow (Measure_dimless, 4))))
       )) |}]
;;
          ...
          (Measure_pow (
             (Measure_prod (
                (Measure_prod (Measure_dimless,
                   (Measure_pow (
                      (Measure_div (
                         (Measure_div (Measure_dimless, Measure_dimless)),
                         Measure_dimless)),
                      2))
                   )),
                (Measure_prod (Measure_dimless,
                   (Measure_prod (Measure_dimless,
                      (Measure_prod ((Measure_pow ((Measure_ident "rQ1_R"), 7)),
                         Measure_dimless))
                      ))
                   ))
                )),
