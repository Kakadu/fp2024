...
let run_single options =
  ...
  let run text =
    ...
    | Ok ast ->
      (* Infer *)
      if options.do_not_type = false then print_endline "Inference can't be done. To turn off this message, run REPL with --do-not-type option";
      match eval ast with
      | Ok (env, out_lst) ->
  ...
  let () =
    ...
    parse
      ...
      ;( "--no-hi"
        ...
        , "Turn off greetings message" )] (fun _ -> exit 1) "REPL"
  in
  if options.greet_user then
