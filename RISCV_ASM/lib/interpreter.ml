...
let get_address12_value program = function
  | ImmediateAddress12 value -> value
  | LabelAddress12 label -> resolve_label program label
;;

let get_address20_value program = function
  | ImmediateAddress20 value -> value
  | LabelAddress20 label -> resolve_label program label
;;

let get_address32_value program = function
  | ImmediateAddress32 value -> value
  | LabelAddress32 label -> resolve_label program label
;;
...
and execute_instruction state instr program =
  ...
  | Jalr (rd, rs1, imm) ->
    ...
    let new_pc = val_rs1 + imm_value in
  ...
  | J imm ->
    let imm_value = get_address20_value program imm in
    let label_idx = resolve_label program (string_of_int imm_value) in
    if label_idx = -1
    then state
    else (
      let new_state = set_register_value state X0 (state.pc + 1) in
...
let show_state state =
  let registers_str =
    StringMap.fold
      (fun reg value acc -> acc ^ Printf.sprintf "%s: %d\n" reg value)
      state.registers
      ""
  in
  let pc_str = Printf.sprintf "PC: %d" state.pc in
