...
type instruction =
    ...
    | Sb of register * immediate12 * register (** Store Byte. M[rs1 + imm][0:7] = rs2[0:7] *)
    | Sh of register * immediate12 * register (** Store Half. M[rs1 + imm][0:15] = rs2[0:15] *)
    | Sw of register * immediate12 * register (** Store Word. M[rs1 + imm][0:31] = rs2[0:31] *)
    | Beq of register * register * immediate12 (** Branch ==. if (rs1 == rs2) PC += imm. PC is a program counter *)
    ...
    | Bltu of register * register * immediate12 (** Branch < (Unsigned). if (rs1 < rs2) PC += imm. *)
    | Bgeu of register * register * immediate12 (** Branch >= (Unsigned). if (rs1 >= rs2) PC += imm. *)
    | Jal of register * immediate20 (** Jump and Link. rd = PC + 4; PC += imm. 4 bytes = 32 bits - instuction size *)
    | Jalr of register * register * immediate12 (** Jump and Link Register. rd = PC + 4; PC = rs1 + imm *)
    | Lui of register * immediate20 (** Load Upper Immediate. rd = imm << 12 *)
    | Auipc of register * immediate20 (** Add Upper Immediate to PC. rd = PC + (imm << 12) *)
    ...
    | Mulhsu of register * register * register (** Multiply High (Signed * Unsigned). rd = (rs1 * rs2)[63:32] *)
    | Mulhu of register * register * register (** Multiply High (Unsigned * Unsigned). rd = (rs1 * rs2)[63:32] *)
    ...
    (** RV64I instructions *)
    | Lwu of register * register * immediate12 (** Load Word (Unsigned). rd = M[rs1 + imm][0:31] *)
    ...
    | Srlw of register * register * register (** Shifl Right Logical Word. rd = (rs1 >> rs2)[31:0] *)
    | Sraw of register * register * register (** Shifl Right Arithmetical Word. rd = (rs1 >> rs2)[31:0] *)
    (** RV64M instructions *)
    ...
    | Divuw of register * register * register (** Division Word (Unsigned). rd = (rs1 / rs2)[31:0] *)
