...
type statement = 
    ...
    | IfStatement of expression * statement list * statement list                                       (* if x > 0 then (..) else (..) *)
    | DefinitionStatement of def_type * placement * identifier * identifier list * statement list       (* let [rec] f = (..) *)
[@@deriving show { with_path = false }]
...
**)
type program = identifier * statement list
[@@deriving show { with_path = false }]
