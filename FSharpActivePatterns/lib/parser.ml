...
let p_expr =
  skip_ws
  *> fix (fun p_expr ->
    let atom = choice [ p_var; p_int; p_bool; p_parens p_expr; p_semicolon_list p_expr] in
    ...
    let cons_list = p_cons_list comp_or <|> comp_or in 
    let option = p_option cons_list <|> cons_list in
    option)
;;
